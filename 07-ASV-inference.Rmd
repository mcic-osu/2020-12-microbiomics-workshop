...
The `truncLen` argument defines the read lengths (for *forward* and *reverse* reads)
...
in order to maximize the number of paris that will merge in a subsequent step.

It is also suggested to trim first 10 nucleotides of each read,
...
We will now save the dereplicated sequence objects to RDS files,
so we can remove these objects from our environment:
...
Build the final phyloseq object (we will not do this now, but a phylogenetic tree can also be added to the phyloseq object):
