...
```{r knitr_options, echo=FALSE, purl=FALSE}
...
Decontam can't handle NA's or 0's,
...
MCIC's DNA concentrations are not very accurate,
so I chose a relaxed p-value (0.2) to reject the null hypothesis.
...
I expect that if we were to use more accurate measures of DNA concentration
(e.g. fluorometric or gel electrophoresis with a standard curve),
we would find a stronger association (lower p-value) between DNA concentration
...
The V4 rDNA region is conserved within certain bacteria, archaea, chloroplasts,
mitochondria, and eukaryotes.
Since we are only interested in bacteria and archaea,
we should remove these other taxa from our object.
...
However, non-experimental samples are important for evaluating the quality of
your reagents, consistency across sequencing runs and contaminaiton.
...
Let's begin to investigate our samples.
...
Technical replicates are also important for quality control of runs and
understanding reproducibility of the methodology. 
...
Let's take a look at our `phyloseq` object:
...
variance stabilizing transformation normalized data (via DESeq2) to identify differentially abundant taxa. For more details, read
...
Use `?vst` or read [Anders and Huber 2010](http://dx.doi.org/10.1186/gb-2010-11-10-r106)
...
```{r}
cor <- cor.test(dp_by_asv$ct_sums, dp_by_asv$unique_asvs, method = "spearman")
cor
```
...
This is a high proportion of the total number taxa are observed in 101-S1:
...
If your *phyloseq* object lacks a phylogenetic tree,
