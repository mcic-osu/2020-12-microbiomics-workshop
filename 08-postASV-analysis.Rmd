...
We can see that the lowest count sample, `r names(sums[order(sums)][1])`, has `r min(sums)` taxa count(s). To remove uninformative samples, we will only keep those with over 1,000 counts.We will also remove a technical replicate. Technical replicates are also important for quality control of runs and understanding reproducibility of the methodology. 
