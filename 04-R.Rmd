...
- An account at OSC with access to an OSC Project.   
  *While these materials can be used either in a local R and RStudio installation
  or using RStudio Server at the Ohio Supercomputer Center (OSC),
  we recommend you to use OSC as we will also use OSC for other elements
  of this workshop.*
...
- In the blue top bar, click the `Interactive Apps` dropdown menu.
- All the way in the bottom of the dropdown, click `RStudio Server (Owens and Pitzer)`.
- In the submission form, keep all the defaults, unless you want to select
  a particular project, and then click `Launch`.
- Wait until the top bar of the little job box turns green and says "Running",
  and then click the blue `Connect to RStudio Server` button in the bottom of the Box.
- You're in!
...
Let's start by performing an amazing subtraction:
...
(The `[1]` in the results is a bit weird; you can ignore that for now.)

You can generally use R as a calculator in this way, with standard symbols:

```{r calculations, results='show'}
20 * 13   # Multiplication
20 / 13   # Division
```
...
OK, so this was easy, but in practice,
you will type incomplete commands without realizing you did so.
Just remember that when you see the `+` prompt,
you are missing something in your command:
oftentimes, you have forgotten a closing parenthesis `)`
or accidentally opened up an unwanted opening parenthesis `(`.
...
```{r gibberish, results='show', error=TRUE}
Awesome
```
...
But there won't be an easy way to keep track of what we've done.

It's best to write your code in separate files, i.e. "scripts".
...
*P.S.: Another point to note is that in R you can (and should) always use a
...
and share it with others without worrying about whether or not the underlying scripts
will still work.
...
* Under the `File` menu,
  ...
  choose `Existing directory`, then
  click the `Browse` and find your `r folder_name` folder.

* Click on `Create Project`.

Now, RStudio will reload with the newly created Project open.
One consequence of this is that your working directory should be the Project's
directory. Let's see what our working directory is using `getwd()`:

```r
getwd()
```

From now on, we will avoid the temptation (?) to change our working directory,
...
- Click `File` > `Save As` to save the script.   
  Save it in your `scripts` subfolder
  (e.g. `~/2020-12_microbiomics-workshop/scripts/`)
  using the file name `intro-to-R.R`.
  

### Interacting with the R console from your script

I recommend always *typing your commands into a script*,
which you'll save for later reference,
and then executing the commands from there.

We want to make sure to save our wonderful subtraction,
...
Now place the cursor on the line with `20 - 13` and type
...
You can use `#` both at the start of a line or anywhere in the line, after actual code.
...
current objects with `ls()`. RStudio also shows the objects in the Environment panel.
...
Earlier, we managed to divide `20` by `13` and got `r 20/13`,
but what if we wanted to go even further and *round* the result?
...
(usually, the primary argument is converted by the function, like here).

### Using named arguments

Functions can have more than one argument, and some of them may have default values.

There are some functions that take many arguments and you can imagine that it might
get confusing trying to keep them in order.
In that case, it is often better to explicitly name the arguments.
...
I generally focus on *Usage* and *Arguments*, and then on *Examples* at the bottom.

Otherwise, for instance when you don't know a function's name, or even whether
a function may or may not exist, Google is your friend!
Try typing "rounding a number in r", and see if you get useful results.
...
As mentioned, all of a vector's elements are the same type of data.
...
These classes are different types of **atomic vectors**, R's simplest **data type**.
...
What happens if we try to mix vector types (e.g., "character and numberic"
in a single vector?
...
Let's say we wanted to convert our weights to pounds,
which means that ...

```{r}
# weights <- c(50, 60, 65, 82)     # Still working with the same weights vector
...
This means that in this case, each element in the vector `weights` will be multiplied by 5.
...
<span style="color: #eb4034">TODO: EXPAND AND USE WEIGHT CONVERSION AS AN EXAMPLE</span>


### Data structures in R

Vectors, in turn, are one of the many **data structures** that R uses.
...
- **`data.frame`** -- A rectangular data structure where each column can be a different data type, see below!
...
We will look at a dataset with the species and weight of animals caught
in plots in a study area in Arizona over time.
This data is available on the web, and we can read it straight from there:
  
  ```{r read_csv_from_web}
surveys <- read.csv("http://kbroman.org/datacarp/portal_data_joined.csv")
```

The data are stored in what's called a **"data frame"**
(formal object type: `data.frame`).
It's a big rectangle, with rows being observations and columns being variables.
In a data frame, different columns can be different types (numeric, character, etc.),
but all columns have the same length.
...
* **Size:**
  ...
  * `length()` -- for a dataframe, returns the number of columns. Very commonly used for vectors, for which it will return the number of elements.
...
Note: most of these functions are "generic",
...
As R was designed to analyze datasets,
it includes the concept of missing data
...
"S4" objects are most certainly an advanced R topic,
but because many bioinformatics packages use them,
we will briefly discuss them here.
In a nutshell, S4 objects allow for complicated, multifaceted datasets
(such as multiple dataframes with distinct but related data as well as with metadata)
to be represented in a single object.

<span style="color: #eb4034">TODO: EXPAND</span>
...
- To save data frames and other tabular data, use `write.table()` in base R
  or one of *readr*'s [writing functions](https://readr.tidyverse.org/reference/write_delim.html).
